...
class Bot:
    ...
    def _opp_value(self, obs, quote=None):
        ...
        An accepted-trade one-sided bound (opponent's revealed sum AT
        THE TIME of a past accept) was tried here and reverted: it is
        real, hard information about k_theirs AS IT STOOD AT THAT PAST
        ROUND, but k_theirs keeps growing every subsequent round as
        more of the opponent's hand reveals (REVEAL_PER_ROUND new,
        mean-zero coins each round) -- a bound on the smaller past
        quantity does not carry forward as a valid bound on the larger
        current one, and using it as a hard clamp on the CURRENT
        estimate is simply incorrect once even one round has passed.
        Measured as a real regression (-2 to -7/match across all three
        reference bots) once this was caught by direct trace rather
        than trusted on the strength of the derivation alone -- a
        reminder that "logically derived" and "correctly derived" are
        not the same thing without checking the quantity's own
        dynamics over time, not just its instantaneous definition."""
    ...
    def _te_bid(self, obs, v):
        ...
        A further correction, found by tracing this bot's own live
        auctions against adaptive_bidder: comparing v against the FULL
        undiscounted average of every remaining round systematically
        starves any round whose realised draw happens to be below that
        average, even in round 1 with 4 full rounds still ahead of it
        (e.g. FORESIGHT at 0.56 vs a ~2.6-average future reserve rounds
        the share to ~6%, a 1-2 TE bid, when the round is still clearly
        worth contesting on ceiling_1's own terms). A round with N total
        slots remaining (this one included) is owed AT LEAST a uniform
        1/N share of what's left as a floor, regardless of how its own
        realised value compares to the unconditional future average --
        share is the LARGER of the value-proportional share and that
        uniform floor, never just the value-proportional one alone. A
        genuinely high-value round is unaffected by this (its value
        share already exceeds the uniform floor on its own, verified:
        STEALTH_ROCK at 8.71 in round 1 gets share=0.51 either way), so
        this only ever loosens ceiling_2 where it was needlessly tight,
        never loosens it where the original conservatism was doing real
        work.
        """
        ...
        rounds_left = 6 - obs.round  # this round plus every round still to come
        future_reserve = sum(
            _EXPECTED_VALUE_BY_ROUND.get(r2, 0.0)
            for r2 in range(obs.round + 1, 6)
        )
        denom = v + future_reserve
        value_share = 1.0 if denom <= 0 else v / denom
        uniform_share = 1.0 / rounds_left if rounds_left > 0 else 1.0
        round_share = max(value_share, uniform_share)
