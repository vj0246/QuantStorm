...
class Bot:
    ...
    def respond(self, obs, quote, turn):
        ...
        # Last-turn forced-fill shift check: a COUNTER on the final
        # turn always forces (there is no turn after it to accept).
        # Under MIDPOINT_SIDE_RULE="last_quoter_sells" (the configured
        # rule -- checked live below, not assumed), the forcer is
        # always the last quoter and therefore always SHORT on the
        # resulting fill, so a shift that pushes the price UP is in
        # our favor when we force. fill_shift() in engine.py computes
        # shift = short_holder_magnitude - long_holder_magnitude, i.e.
        # our own TRICK_ROOM/STEALTH_ROCK magnitude minus the
        # opponent's (obs.powers_theirs is directly visible -- not
        # hidden information -- so this needs no estimate). Net edge
        # from forcing is that shift minus the fixed FORCED_FILL_FEE
        # charged to whoever forces. Only applies when we hold or the
        # opponent holds TRICK_ROOM/STEALTH_ROCK on the last turn, so
        # this is a narrow, well-defined correction, not a general
        # override of the accept logic.
        if is_last_turn and self.config.MIDPOINT_SIDE_RULE == "last_quoter_sells":
            ...
            net_force_shift = my_shift_mag - opp_shift_mag
            net_force_edge = net_force_shift - self.config.FORCED_FILL_FEE
            best_accept_edge = max(edge_buy, edge_sell, 0.0)
            if net_force_edge > best_accept_edge:
                # Pin as close to (ask, ask) as final_cap legally
                # allows: we are short, so the higher the fill
                # midpoint the better, and the fill midpoint is
                # (bid+ask)//2 of whatever we counter with -- pushing
                # bid up toward ask (bounded below by ask - final_cap)
                # maximizes that midpoint within the legal width floor.
