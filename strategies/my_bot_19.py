...
class Bot:
    ...
    def respond(self, obs, quote, turn):
        ...
        # and the resulting fill shifts by the net magnitude of
        # TRICK_ROOM/STEALTH_ROCK we hold, minus the fixed forcing
        # fee charged to whoever forces (fill_shift / FORCED_FILL_FEE
        # in engine.py -- both real, fixed game constants, not
        # estimated). If that net shift beats the best available
        # accept edge, forcing here is the higher-EV action even
        # though an accept was on the table -- this can only apply
        # holding TRICK_ROOM and/or STEALTH_ROCK and only on the last
        # turn, so it is a narrow, well-defined correction, not a
        # general override of the accept logic.
        if is_last_turn:
            my_shift_mag = sum(
            ...
            net_force_edge = my_shift_mag - self.config.FORCED_FILL_FEE
