...
10. Opponent-budget bid cap. obs.te_theirs is genuine current public
    state, not an assumption about their bid this round (the auction
    itself stays blind first-price). Capping our own bid at
    te_theirs + 1 never loses an auction we'd otherwise have won and
    strictly reduces cost whenever their remaining budget, not our own
    valuation, is the binding constraint. Small, consistent, and inert
    (never harmful) rather than opponent-specific tuning -- see _te_bid.

11. FORESIGHT-defense width. An earlier draft special-cased the opening
    quote to obs.spread_cap whenever the opponent held FORESIGHT, on the
    assumption maximum width is the safest response to their edge.
    Tracing through _best_width's own math showed the opposite: a larger
    `unseen` count pulls p_true toward p_base for every width, so the
    obligation term vanishes and the unmodified optimizer already
    collapses to the FLOOR under reduced relative information -- there is
    no separate case to write. Verified in head-to-head and full-matchup
    checks before removing the special case.
...
SHADE = 0.5                 # Bayes-Nash first-price N=2 uniform-IPV shade, derived (section 6)
...
FORCE_MARGIN = 1.0          # required edge of forcing over best accept, in ticks, before forcing
                             # NOT independently derived -- a round margin chosen because a
                             # sweep over {0.0, 0.5, 1.0} moved results by less than noise in
                             # the runs checked. Weaker justification than the other constants
                             # in this file and flagged as such rather than dressed up.
...
class Bot:
    ...
    def _value(self, obs, quote=None):
        """Best estimate of the final score S. Clipped to [-N_COINS, N_COINS]
        -- S is the sum of all N_COINS fair +/-1 coins, so no honest
        estimate of it can fall outside that range regardless of how it
        was built. FORESIGHT's k_hat (section 1) stays inside this range
        by construction (it's a bounded rescale of a bounded sample), but
        the quote-anchor path (section 2) uses a PAST round's raw quote
        midpoint, which was set by the opponent's own logic under a
        smaller coin count and carries no such bound -- adding a larger,
        later-round k_mine on top of a stale anchor can overshoot what's
        physically achievable. This clip doesn't change which signal is
        used or its estimated value; it only prevents a legal-but-
        physically-impossible instruction from being quoted, which the
        engine would otherwise silently reprice for us. Found via the
        backtester's own out-of-range warning against a real opponent,
    ...
    def _te_bid(self, obs, v):
        ...
        # Keep the bid at the model-derived ceiling.  A blanket +1 TE
        # tie-break would systematically overpay in states where the
        # opponent's bid distribution is unknown, so it would introduce
        # opponent-model dependence rather than a distribution-free EV edge.

        # Ceiling 3: obs.te_theirs is genuine, current public state
        # (confirmed against the Obs dataclass and RULEBOOK -- not
        # inferred or assumed). The auction is blind and first-price
        # (winner pays their own bid, ties broken by a coin flip, no
        # refund), but blind does not mean the opponent's BUDGET is
        # hidden -- only their bid this round is. If they cannot legally
        # bid more than te_theirs, there is no return to bidding further
        # above that: bidding te_theirs + 1 wins whenever they'd have won
        # by bidding their entire remaining budget, at a strictly lower
        # cost than ceilings 1-2 would otherwise pay. This does not
        # replace ceilings 1-2 -- it only tightens the bid when their
        # budget is the binding constraint, never loosens it. Tested
        # against the reference bot: a small, consistent improvement
        # (~+0.3 ticks/deal), and inert (no effect) against opponents
        # whose remaining TE isn't the tightest constraint.
        capped = min(round(bid_te), obs.te_theirs + 1)
        return max(0, min(capped, obs.te_mine))
    ...
    def quote(self, obs):
        ...
        # Adverse selection when they hold FORESIGHT: an earlier draft
        # special-cased this to obs.spread_cap on the theory that maximum
        # width is the safest posture. Checking _best_width's own math
        # showed that's backwards -- feeding it a larger `unseen` count
        # makes p_true converge toward p_base for every width, so the
        # obligation-EV term vanishes and the search collapses to the
        # FLOOR, not the cap. Letting _best_width run unmodified (no
        # special case) beat the forced-cap version in every check run
        # (+0.5 to +1.0 ticks/deal, both head-to-head against a
        # FORESIGHT-holding opponent and in full matchups), so there is no
        # separate branch here: my_unseen already reflects whatever we
        # know, and the optimizer already responds correctly to reduced
        # relative information without being told to.
    ...
    def respond(self, obs, quote, turn):
        ...
        if turn >= self.config.N_TURNS:
            ...
            # The engine sanitises ("COUNTER", ask, ask) to the minimum legal
            # width, so the forced settlement is the integer midpoint of
            # [ask - final_cap, ask].  Match engine.trade_round exactly,
            # including odd-width integer flooring.
            final_cap = int(obs.final_cap)
            realized_midpoint = (2 * ask - final_cap) // 2
            force_payoff = (
                (realized_midpoint - v)
                + shift_mine
                - shift_theirs
        ...
        # Quote around the estimated fair value, but do not surrender the
        # whole edge by placing the counter exactly at round(v).  The skew
        # is state-dependent and symmetric: when we are the prospective
        # buyer, lower the quoted center; when we are the prospective seller,
        # raise it.  This preserves the no-opponent-model property while
        # extracting one tick of price improvement whenever the legal range
        # permits it.
