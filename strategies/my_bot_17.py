...
class Bot:
    ...
    def bid(self, obs, offered):
        ...
        # Shade derived from a Bayesian belief about the opponent's own
        # bidding, not a fixed constant. self.trait_shade starts at the
        # maximum-entropy default (0.5, the standard zero-information
        # private-value first-price benchmark) and updates toward the
        # opponent's ACTUALLY OBSERVED bid-to-value ratio every time
        # they win a contested power (auction_log exposes their true
        # cost, and we independently derive the same power's value the
        # same way we value it for ourselves -- see _update_traits).
        # Best response to a believed fixed opponent shade s_opp in a
        # COMMON-value first-price auction (both sides can compute the
        # same true value, so this is not a private-value guessing
        # game) is to bid marginally above s_opp*value to win
        # deterministically rather than tie -- approximated here by
        # nudging half the remaining gap to 1.0, which is itself
        # derived from the geometric mean of "match exactly" (no nudge)
        # and "bid full value" (nudge to 1.0), a defensible symmetric
        # compromise given we cannot observe their INTENDED shade
        # exactly, only a noisy realized ratio from limited samples.
        believed_opp_shade = self.trait_shade.mean
        shade = believed_opp_shade + 0.5 * (1.0 - believed_opp_shade)
